...
from cts.baxus import EmbeddedTuRBO, EmbeddedSTuRBO
from cts.baxus.benchmarks.benchmark_function import Benchmark
from cts.baxus.util.behaviors import SBAxUSBehavior
from cts.baxus.util.behaviors.gp_configuration import GPBehaviour
from cts.baxus.util.data_utils import join_data
from cts.baxus.util.projections import AxUS, ProjectionModel
from cts.baxus.util.utils import (
...
class SBAxUS(EmbeddedSTuRBO):
    ...
    def __init__(
            ...
            n_init: int,
            max_evals: int,
            behavior: SBAxUSBehavior = SBAxUSBehavior(),
        ...
        assert (
                self.R_init > 2 * self.R_min
        ), f"Initial R {self.R_init} has to be larger than two times the minimum R {self.R_min}."
    ...
    def R_min(self) -> float:
        """
        The minimum base sigma of the trust region.

        Returns: The minimum base sigma of the trust region.
    ...
    def R_max(self) -> float:
        """
        The maximum base sigma of the trust region.

        Returns: The maximum base sigma of the trust region.
    ...
    def R_init(self) -> float:
        """
        The initial base sigma of the trust region.

        Returns: The initial base sigma of the trust region.
    ...
    def _restart(self, sigma: float = None, R: float = None) -> None:
        """
        Reset TR observations, reset counter, reset base sigma

        Args:
            sigma: new base sigma after resetting, if not set, sigma_init will be used.
    ...
    def failtol(self) -> float:
        ...
        use_custom_failtol = True

        if self.target_dim == self.input_dim:
            if use_custom_failtol:
                if not hasattr(self, 'budget_used_before_input_dim'):
                    self.budget_used_before_input_dim = deepcopy(self.n_evals)
                k = np.ceil(-np.log2(self.R_min/self.R_init)) # fails to reach R_min
                custom_ft_max = np.ceil(0.5 * (self.max_evals - self.budget_used_before_input_dim) / k) 
                ft_max = np.min([ft_max, custom_ft_max])
            return ft_max
        ...
        R_init = self.behavior.initial_rho * np.sqrt(self.target_dim)

        gamma = 2 * math.log(self.R_min / R_init, 0.5)
        if gamma == 0:
            return ft_max
    ...
    def _adjust_sigma(self, fX_next) -> None:
        """
        Adjust the base sigma of the current trust region depending on the outcome of the next evaluation.
        If the next evaluation is better than the current, increase success count and potentially increase TR base sigma.
        Otherwise, increase fail count and potentially decrease TR base sigma.
        ...
        """
        debug(
            f"eval {self.n_evals}: sigma = {self.sigma}, failcount = {self.failcount} (failtol = {self.failtol}), "
        ...
        self._log_property("sigma_history", f"{self.n_evals}:{self.sigma}")
    ...
    def _resample_and_restart(self, n_points: int, sigma: float = None, R: float = None) -> None:
        """
        Resample new initial points and reset algorithm.

        Args:
            n_points: number of new initial points
            sigma: new base sigma after resetting

        Returns: None

        """
        # Initialize parameters
        self._restart(sigma=sigma, R=R)
        ...
        while self.n_evals < self.max_evals and not self._optimum_reached():
            ...
            if len(self._fX) > 1:
                ...
                if dims_and_bins:  # if we have a remaining-splitting dim
                    ...
                    self.sigma = self.behavior.initial_sigma
                    self.R = self.behavior.initial_rho * np.sqrt(self.target_dim)
                    self.sampler.d = self.target_dim
                    ...
                    self._resample_and_restart(
                        n_points=self.n_init, sigma=self.sigma_init, R=self.R_init
                    )
                    self._axus_change_iterations.append(self.n_evals)
                ...
                self._resample_and_restart(self.n_init, self.sigma_init, self.R_init)
                fbest = self._fX.min()
