...
from cts.baxus import EmbeddedTuRBO, BAxUS, SBAxUS
from cts.baxus.benchmarks import run_and_plot, EffectiveDimBenchmark, EffectiveDimBoTorchBenchmark, RandomSearch
from cts.baxus.util.behaviors import BaxusBehavior, EmbeddedTuRBOBehavior, SBAxUSBehavior
from cts.baxus.util.behaviors.gp_configuration import GPBehaviour
from cts.baxus.util.exceptions import ArgumentError
from cts.baxus.util.parsing import parse, embedding_type_mapper, acquisition_function_mapper, mle_optimization_mapper, \
    fun_mapper
from cts.baxus.util.utils import star_string
...
def main(argstring: List[str]) -> None:
    ...
    for i, (k, comb) in enumerate(combs.items()):
        ...
        if "cts-baxus" == args.algorithm:
            # *** BAxUS ***
            info("***CTS-BAxUS***")
            behavior = SBAxUSBehavior(
                n_new_bins=new_bins_on_split,
                acquisition_function=acquisition_function,
                embedding_type=bin_sizing_method,
                adjust_initial_target_dim=adjust_initial_target_dim,
            ...
            conf_name = (
                f"sbaxus_{behavior}_{gp_behaviour}"
            )
            # run_dir = os.path.join(
            #     function_dir,
            #     str(adler32(conf_name.encode("utf-8"))),
            # )
            run_dir = function_dir
            ...
            run_and_plot(m=sbaxus, repetitions=repetitions, directory=run_dir)
            del sbaxus
