...
from cts.baxus.util.behaviors.gp_configuration import MLLEstimation, GPBehaviour
from cts.baxus.util.gp_utils import (
    initializer_factory,
    mle_optimization,
...
class NormalizingCylindricalKernel(CylindricalKernel):
    ...
    def forward(self, x1: torch.Tensor, x2: torch.Tensor, diag: Optional[bool] = False, **params) -> torch.Tensor:
        ...
        x1_ /= math.sqrt(x1_.shape[-1] + 1e-6)
        x2_ /= math.sqrt(x2_.shape[-1] + 1e-6)
...
class BOCKGP(SingleTaskGP):
    ...
    def __init__(
        ...
        super(BOCKGP, self).__init__(
            train_x,
            torch.unsqueeze(train_y, 1) if train_y.ndim == 1 else train_y,
            likelihood,
        )
        self.likelihood = likelihood
        matern_radial_kernel = MaternKernel(
            nu=2.5,
            # ard_num_dims=1,
            # lengthscale_constraint=lengthscale_constraint,
        )
        base_kernel = NormalizingCylindricalKernel(
            num_angular_weights=3,
            radial_base_kernel=matern_radial_kernel
        )
...
def train_bock_gp(
    ...
    if res_hypers:
        ...
        hyperparameter_grid["covar_module.base_kernel.radial_base_kernel.lengthscale"] = (
            0.005,
            10.0,
            0.5,
        )
