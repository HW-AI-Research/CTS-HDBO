...
from cts.baxus.benchmarks.other_methods import OptimizationMethod
from cts.baxus.gp import train_gp
from cts.baxus.util.acquisition_function_types import AcquisitionFunctionType
from cts.baxus.util.behaviors import EmbeddedSTuRBOBehavior
from cts.baxus.util.behaviors.gp_configuration import GPBehaviour
from cts.baxus.util.projections import AxUS
...
class EmbeddedSTuRBO(OptimizationMethod):
    """
    ...
    def __init__(
            ...
            n_init: int,
            max_evals: int,
            behavior: EmbeddedSTuRBOBehavior = EmbeddedSTuRBOBehavior(),
        ...
        # epsilon = 1e-3
        # self._sigma_min = epsilon / np.sqrt(self.target_dim)
        
        # Trust region sizes
        # self._sigma_min = self.behavior.min_sigma
        # self._sigma_max = self.behavior.max_sigma
        self._sigma_init = self.behavior.initial_sigma
        self._sigma_max = self.behavior.max_sigma

        rho = self.behavior.initial_rho
        self._R_init = rho * np.sqrt(self.target_dim)
        self._R_min = self.behavior.min_rho * np.sqrt(self.target_dim)
        self._R_max = self.behavior.max_rho * np.sqrt(self.target_dim)

        self.model_space = True
        ...
        self.sampler = TMVN_CylindricalSampler(d=self._target_dim, sigma_init=self.sigma_init, device=self.device)
    ...
    def n_cand(self) -> int:
        ...
        n_cand = min(100 * self._target_dim, 5000)
        # return min(n_cand, 450_000 // self._target_dim)
        return n_cand
    ...
    def _resample_and_restart(self, n_points: int, R: float = None, sigma: float = None) -> None:
        """
    ...
    def _restart(self, R, sigma: Optional[float] = None) -> None:
        """
    ...
    def _adjust_sigma(self, fX_next: np.ndarray) -> None:
        """
        ...
        self._log_property("sigma", f"{self.n_evals}:{self.sigma}")
    ...
    def _create_candidates(
        ...
        tr_bounds = torch.cat([-torch.ones(1, self.target_dim, **self.tkwargs), torch.ones(1, self.target_dim, **self.tkwargs)], dim=0)
