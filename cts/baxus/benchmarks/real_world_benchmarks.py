...
from cts.baxus.benchmarks import SyntheticBenchmark, EffectiveDimBenchmark
...
class MoptaSoftConstraints(SyntheticBenchmark):
    ...
    def __init__(
        ...
        binary_path = self._executable_path()
        if not binary_path.exists():
            self._retrieve_bin(mopta_full_path=binary_path)
    ...
    def _executable_path(self) -> Path:
        ...
        mopta_full_path = Path(__file__).parents[2] / 'bo_utils' / 'mopta08' / mopta_exectutable        
        
        return mopta_full_path
        
    def _retrieve_bin(self, mopta_full_path: Path):
        import stat
        mopta_full_path.parent.mkdir(exist_ok=True)
        mopta_exectutable = mopta_full_path.name
        urllib.request.urlretrieve(
            f"https://leonard.papenmeier.io/assets/mopta08/{mopta_exectutable}",
            mopta_full_path)
        os.chmod(mopta_full_path, stat.S_IXUSR)
