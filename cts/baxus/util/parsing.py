...
from cts.baxus.benchmarks.synthetic_benchmark_functions import (
...
from cts.baxus.util.behaviors.gp_configuration import MLLEstimation
from cts.baxus.util.behaviors.embedding_configuration import EmbeddingType
...
def parse(args):
    ...
    parser.add_argument(
        ...
        type=str,
        default="baxus",
        choices=["baxus", "cts-baxus", "embedded_turbo_target_dim", "embedded_turbo_effective_dim",
...
def benchmark_loader(bench: str, args: Namespace):
    ...
    if bench == "lasso-leukemia":
        from cts.baxus.benchmarks.real_world_benchmarks import LassoLeukemiaBenchmark

        _fun_mapper[bench] = LassoLeukemiaBenchmark

    if bench == "lasso-breastcancer":
        from cts.baxus.benchmarks.real_world_benchmarks import LassoBreastCancerBenchmark

        _fun_mapper[bench] = LassoBreastCancerBenchmark

    if bench == "lasso-dna":
        from cts.baxus.benchmarks.real_world_benchmarks import LassoDNABenchmark

        _fun_mapper[bench] = LassoDNABenchmark

    if bench == "lasso-diabetes":
        from cts.baxus.benchmarks.real_world_benchmarks import LassoDiabetesBenchmark

        _fun_mapper[bench] = LassoDiabetesBenchmark

    if bench == "lasso-rcv1":
        from cts.baxus.benchmarks.real_world_benchmarks import LassoRCV1Benchmark

        _fun_mapper[bench] = LassoRCV1Benchmark
    if bench == "lasso-simple":
        from cts.baxus.benchmarks.real_world_benchmarks import LassoSimpleBenchmark

        _fun_mapper[bench] = LassoSimpleBenchmark
    if bench == "lasso-medium":
        from cts.baxus.benchmarks.real_world_benchmarks import LassoMediumBenchmark

        _fun_mapper[bench] = LassoMediumBenchmark
    if bench == "lasso-high":
        from cts.baxus.benchmarks.real_world_benchmarks import LassoHighBenchmark

        _fun_mapper[bench] = LassoHighBenchmark
    if bench == "lasso-hard":
        from cts.baxus.benchmarks.real_world_benchmarks import LassoHardBenchmark

        _fun_mapper[bench] = LassoHardBenchmark

    if bench == "mopta08":
        from cts.baxus.benchmarks.real_world_benchmarks import MoptaSoftConstraints

        _fun_mapper[bench] = MoptaSoftConstraints

    if bench == "svm":
        from cts.baxus.benchmarks.real_world_benchmarks import SVMBenchmark
