...
from .problems.effective_dim import BraninCurrinEffectiveDim
from logging import info, debug, warning

def get_multiobj_fn(evalfn, dim, max_reference_point, tkwargs):
    bounds = torch.empty((2, dim), **tkwargs)
    constraints = None
    objective = None
    ...
    if evalfn == "C2DTLZ2":
        ...
        f, bounds = get_rover_fn(
            dim, force_goal=False, force_start=True, **tkwargs
        )
        ...
        constructor_map = {
            ...
            "BraninCurrin_EffectiveDim": BraninCurrinEffectiveDim,
        }
    ...
    return f, num_outputs, bounds, num_objectives, objective, constraints
...
def get_indices_in_hypersphere(
    X_center: Tensor, X: Tensor, R: float, eps: float = 1e-10
) -> Tensor:
    ...
    """
    return (torch.linalg.norm(X - X_center, dim=1) <= R + eps).nonzero().view(-1)
...
def get_fitted_model(
    ...
    if X.is_cuda:
        info(f"after fitting: {torch.cuda.memory_allocated(X.device) / (1000 ** 3)}")
    return model
