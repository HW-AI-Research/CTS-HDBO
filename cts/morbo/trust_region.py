...
class TurboHParams:
    ...
    max_centers: Optional[int] = None # can be used to avoid memory issues with TMVN sampler
...
class TrustRegion(ABC, Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.tr_hparams.sampler == 'tmvn_cyl':
            self.sampler = TMVN_CylindricalSampler(d=self.dim, device=bounds.device, sigma_init=self.tr_hparams.sigma_init)
        else:
            self.sampler = None
    ...
    @property
    def radius(self):
        R = torch.sqrt(torch.tensor(self.dim, **self.tkwargs)) * self.length
        return R
    ...
    def update_model(self) -> bool:
        ...
        if self._check_for_updates_to_model_training_data():
            ...
            if self.X_center is not None:
                if self.tr_hparams.spherical:
                # if False:
                    coeff = self.radius*torch.ones(self.dim, **self.tkwargs) # 2R
                    offset = self.X_center.squeeze()
                    intf2 = AffineInputTransform(d=self.dim, coefficient=coeff, offset=offset)
    ...
    def update(
        ...
        **kwargs,
        ...
        if self.tr_hparams.verbose and X_new is not None:
            info(f"Num points in TR: {self.X.shape[0]}")
            info(f"length: {self.length}")
        return False
    ...
    def _update_training_data(self, X_all: Tensor, Y_all: Tensor) -> None:
        ...
        if self.tr_hparams.trim_trace:
            if self.tr_hparams.spherical:
                tr_indices = get_indices_in_hypersphere(
                    X_center=self.X_center_normalized,
                    X=X_all_normalized,
                    # used 2x the TR candidate space for model fitting
                    R=2*self.radius, # use 2R since perturbation centers may live on edge of TR
                )
            else:
    ...
    def _reset_counters(self) -> None:
        self.n_successes.zero_()
        self.n_failures.zero_()
        if (self.sampler is not None) and (self.length < 2.0):
...
class ScalarizedTrustRegion(TrustRegion):
    ...
    def _update_center_and_best_points(
        ...
        if self.tr_hparams.verbose and (
            ...
            info(f"New metrics for center: {self.objective(self.Y_center)}")
    ...
    def _update_center_and_best_points(
        ...
        if X_center is not None:
            ...
            if self.X_center_normalized is not None:
                if self.tr_hparams.spherical:
                    indices_in_tr = get_indices_in_hypersphere(
                        self.X_center_normalized, pareto_X_normalized, R=self.radius
                    )
                else:
        ...
        if Y_center_prev is not None:
            if (self.sampler is not None) and (not torch.equal(Y_center_prev.to(self.Y_center), self.Y_center)):
                debug("====> Resetting Cache!")
                self.sampler.reset_cache()
        ...
        if obj.shape[0] > 0:
            ...
            if pareto_mask[-(obj.shape[0]) :].any():
                ...
                if new_hv > (1 + self.tr_hparams.eps) * self.hv:
                    info(f"SUCCESS: hv ratio: {new_hv/self.hv}")
                    return True
        return False
