...
from .sturbo_1 import STurbo1
from .utils import from_unit_cube, latin_hypercube, to_unit_cube
from logging import info, debug, warning

class STurboM(STurbo1):
    ...
    def optimize(self):
        ...
        for i in range(self.n_trust_regions):
            ...
            if self.verbose:
                fbest = fX_init.min()
                info(f"TR-{i} starting from: {fbest:.4}")
                sys.stdout.flush()
            ...
            for i in range(self.n_trust_regions):
                ...
                if len(idx_i) > 0:
                    ...
                    if self.verbose and fX_i.min() < self.fX.min() - 1e-3 * math.fabs(self.fX.min()):
                        ...
                        info(f"{n_evals}) New best @ TR-{i}: {fbest:.4}")
                        sys.stdout.flush()
            ...
            for i in range(self.n_trust_regions):
                if self.length[i] < self.length_min:  # Restart trust region if converged
                    ...
                    if self.verbose:
                        ...
                        info(f"{n_evals}) TR-{i} converged to: : {fbest:.4}")
                        sys.stdout.flush()
                    ...
                    if self.verbose:
                        ...
                        info(f"{n_evals}) TR-{i} is restarting from: : {fbest:.4}")
                        sys.stdout.flush()
